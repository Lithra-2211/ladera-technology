...
export const DealsPage = () => {
  ...
  const [dealFormStage, setDealFormStage] = useState('Proposal Sent');
  ...
  const handleOpenConvertModal = (lead: LeadItem) => {
    ...
    setDealFormStage('Proposal Sent');
      ...
      try {
        await fetch(`/rest/leads/${lead.id}`, {
          ...
          body: JSON.stringify({ 
            isQualifiedLead: true, 
            status: 'Qualified',
            pipelineStage: dealFormStage,
            estimatedDealValue: dealFormAmount,
            expectedCloseDate: dealFormCloseDate
          }),
        });
      } catch (err) {
        console.error('Failed to update lead to deal status:', err);
      }
  ...
  const handleDeleteLead = async (id: string) => {
    ...
    try {
      ...
      if (res.ok) {
        setLeads(prev => prev.map(item => item.id === id ? { ...item, isActive: false } : item));
        showToast(`Lead ${id} successfully moved to trash!`);
      } else {
        setLeads(prev => prev.map(item => item.id === id ? { ...item, isActive: false } : item));
        showToast(`Lead ${id} moved to trash locally.`);
      }
    } catch {
      setLeads(prev => prev.map(item => item.id === id ? { ...item, isActive: false } : item));
      showToast(`Lead ${id} moved to trash locally.`);
  ...
  const handleRestoreLead = async (id: string) => {
    if (!window.confirm(`Are you sure you want to restore lead ${id}?`)) {
      return;
    }

    try {
      const res = await fetch(`/rest/leads/${id}`, {
        method: 'PUT',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ isActive: true })
      });
      if (res.ok) {
        setLeads(prev => prev.map(item => item.id === id ? { ...item, isActive: true } : item));
        showToast(`Lead ${id} successfully restored!`);
      } else {
        setLeads(prev => prev.map(item => item.id === id ? { ...item, isActive: true } : item));
        showToast(`Lead ${id} restored locally.`);
      }
    } catch {
      setLeads(prev => prev.map(item => item.id === id ? { ...item, isActive: true } : item));
      showToast(`Lead ${id} restored locally.`);
          ...
          <TableTopBar>
            ...
            <TableControls>
              ...
              >
                ...
                <option value="Inactive">Deleted Leads</option>
                  ...
                  <FormGrid>
                    ...
                    <FormGroup fullWidth>
                      ...
                      >
                        ...
                        <option value="Closed and Waiting for Contact">Closed and Waiting for Contact</option>
                        <option value="Closed and Contract Signed">Closed and Contract Signed</option>
                      </FormSelect>
                    </FormGroup>
                  </FormGrid>
                </div>
