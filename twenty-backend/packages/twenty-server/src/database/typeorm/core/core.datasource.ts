...
export const typeORMCoreModuleOptions: TypeOrmModuleOptions = {
  ...
  migrations:
    process.env.IS_BILLING_ENABLED === 'true'
      ? [
          `${isJest ? 'src/' : 'dist/src/'}database/typeorm/core/legacy-typeorm-migrations-do-not-add/common/*{.ts,.js}`,
          `${isJest ? 'src/' : 'dist/src/'}database/typeorm/core/legacy-typeorm-migrations-do-not-add/billing/*{.ts,.js}`,
        ]
      : [
          `${isJest ? 'src/' : 'dist/src/'}database/typeorm/core/legacy-typeorm-migrations-do-not-add/common/*{.ts,.js}`,
        ],
